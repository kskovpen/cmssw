...
*/
/*reco::VertexRecoToSimCollection VertexAssociatorByPositionAndTracks::associateRecoToSim(
  const edm::Handle<edm::View<reco::Vertex>> &vCH, const edm::Handle<TrackingVertexCollection> &tVCH, const reco::RecoToSimCollection &trackRecoToSimAssociation) const {
  reco::VertexRecoToSimCollection ret(productGetter_);*/
reco::VertexRecoToSimCollection VertexAssociatorByPositionAndTracks::associateRecoToSim(
											const edm::Handle<reco::VertexCollection> &vCH, const edm::Handle<TrackingVertexCollection> &tVCH, const reco::RecoToSimCollection &trackRecoToSimAssociation) const {
  reco::VertexRecoToSimCollection ret(productGetter_);

//  const edm::View<reco::Vertex> &recoVertices = *vCH;
											   const reco::VertexCollection &recoVertices = *vCH;
  const TrackingVertexCollection &simVertices = *tVCH;
  ...
  for (size_t iReco = 0; iReco != recoVertices.size(); ++iReco) {
    ...
    for (const size_t iSim : simPVindices) {
      ...
      if (zdiff < absZ_ && zdiff / recoVertex.zError() < sigmaZ_ &&
          (!useTiming || (tdiff < absT_ && tdiff / recoVertex.tError() < sigmaT_))) {
        auto sharedTracks = calculateVertexSharedTracks(recoVertex, simVertex, trackRecoToSimAssociation);
        auto fraction = double(sharedTracks) / recoVertex.tracksSize();
        if (sharedTrackFraction_ < 0 || fraction > sharedTrackFraction_) {
          LogTrace("VertexAssociation") << "   Matched with significance " << zdiff / recoVertex.zError() << " "
                                        << tdiff / recoVertex.tError() << " shared tracks " << sharedTracks
...
//          ret.insert(reco::VertexBaseRef(vCH, iReco), std::make_pair(TrackingVertexRef(tVCH, iSim), sharedTracks));
	  ret.insert(reco::VertexRef(vCH, iReco), std::make_pair(TrackingVertexRef(tVCH, iSim), sharedTracks));
        }
      }
    }
  }
...
/*reco::VertexSimToRecoCollection VertexAssociatorByPositionAndTracks::associateSimToReco(
  const edm::Handle<edm::View<reco::Vertex>> &vCH, const edm::Handle<TrackingVertexCollection> &tVCH, const reco::SimToRecoCollection &trackSimToRecoAssociation) const {
  reco::VertexSimToRecoCollection ret(productGetter_);*/
reco::VertexSimToRecoCollection VertexAssociatorByPositionAndTracks::associateSimToReco(
											const edm::Handle<reco::VertexCollection> &vCH, const edm::Handle<TrackingVertexCollection> &tVCH, const reco::SimToRecoCollection &trackSimToRecoAssociation) const {
  reco::VertexSimToRecoCollection ret(productGetter_);
...
//  const edm::View<reco::Vertex> &recoVertices = *vCH;
  ...
  for (size_t iSim = 0; iSim != simVertices.size(); ++iSim) {
    ...
    for (size_t iReco = 0; iReco != recoVertices.size(); ++iReco) {
      ...
      if (zdiff < absZ_ && zdiff / recoVertex.zError() < sigmaZ_ &&
          (!useTiming || (tdiff < absT_ && tdiff / recoVertex.tError() < sigmaT_))) {
        auto sharedTracks = calculateVertexSharedTracks(simVertex, recoVertex, trackSimToRecoAssociation);
        auto fraction = double(sharedTracks) / recoVertex.tracksSize();
        if (sharedTrackFraction_ < 0 || fraction > sharedTrackFraction_) {
          LogTrace("VertexAssociation") << "   Matched with significance " << zdiff / recoVertex.zError() << " "
                                        << tdiff / recoVertex.tError() << " shared tracks " << sharedTracks
...
//          ret.insert(TrackingVertexRef(tVCH, iSim), std::make_pair(reco::VertexBaseRef(vCH, iReco), sharedTracks));
	   ret.insert(TrackingVertexRef(tVCH, iSim), std::make_pair(reco::VertexRef(vCH, iReco), sharedTracks));
        }
      }
    }
  }
